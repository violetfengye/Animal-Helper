<template>
  <div class="students-main-box">
    ...
    <div class="common-module-query-box">
      <div class="module-query-item">
        <span class="module-query-item-title">用户姓名: </span>
        <el-input v-model="queryInfo.name" placeholder="请输入用户姓名" style="width: 220px" clearable/>
      </div>
      <div class="module-query-item">
      ...
      <div class="module-query-item-btn">
        <el-button type="primary" @click="getCommon_users">
  ...
  >
    <el-upload
        drag
        accept=".xlsx"
        :action="`${baseUrl}/uploadFileForCommon_user`"
        name="Common_userTemplateFile"
  ...
  >
    <el-form :model="formData" ref="formRef">
      ...
      <el-form-item label="用户姓名" :label-width="formLabelWidth" prop="name" required>
...
import { User, Common } from "../../api/index.ts"
...
// 获取用户信息
const getCommon_users = () => {
  User.getCommon_usersApi(queryInfo, currentPage.value, pageSize.value).then(response => {
...
// 存储被选中的用户IDs（用于批量激活）
const selectedCommon_usersIds: any = ref([])
...
const handleGetSelected = (selected: any[]) => {
  ...
  selectedCommon_usersIds.value = tempData
}

// 处理批量激活
const handleBatchActive = (studentId?: string) => {
  // 如果有传入用户ID，则直接使用该ID，否则使用选中的用户ID数组
  const ids: string[] = studentId ? [studentId] : selectedCommon_usersIds.value
  // 如果没有选择任何用户，则提示并返回
  if (ids.length === 0) {
    ElMessage.warning('没有选择任何用户，无法进行批量激活！');
    return;
  }
  // 调用批量激活用户的 API
  User.batchActiveCommon_usersApi(ids).then(response => {
    if (response.code !== 200) {
      ElMessage.error(response.msg);
      return;
    }
...
const handleDelete = (rowId: string) => {
  ElMessageBox.confirm(
    ...
    User.deleteCommon_userApi(rowId).then(response => {
      if (response.code !== 200) {
        ElMessage.error(response.msg)
        return
      }
...
const handleUploadSuccess = (response: any) => {
  ...
  if (response.code === 200) {
    ...
    if (failList.length === 0) {
      ...
      msg = `上传的用户信息部分新增成功！有${failList.length}条数据新增失败！行号为：【${rowNumber.toString()}】`
    }
...
const handleSubmit = async (createFormEl: any) => {
  ...
  createFormEl.validate(async (result: boolean) => {
    if (!result) {
      ElMessage.warning('请输入完整的用户信息后重新提交！')
      return
    }
    try {
      ...
      const response = optType.value === 'C'
          ? await User.createCommon_userApi(formData.value)
          : await User.editCommon_userApi({...formData.value, updated_user: userId})
      if (response.code !== 200) {
        ElMessage.error(response.msg)
        return
      }
      ElMessage.success(optType.value === 'C' ? '新增用户信息成功！' : '编辑用户信息成功！')
      getCommon_users()
      dialogVisible.value = false
    } catch (error) {
...
const handleDownload = async () => {
  try {
    const response = await Common.downloadFileApi('UploadCommon_userTemplates.xlsx')
    // 处理下载的文件
    const url = window.URL.createObjectURL(new Blob([response.data], {type: "application/xlsx"}))
    const link = document.createElement('a')
    link.href = url
    // 设置下载文件的文件名
    link.setAttribute('download', '上传模版-用户.xlsx')
