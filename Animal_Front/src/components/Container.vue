    ...
    <div class="container-main-left-box">
      ...
      >
        ...
        <template v-for="menuItem in menuInfo" :key="menuItem.id">
          <el-menu-item
            :index="menuItem.path"
            v-if="menuItem.child_menu.length === 0"
          >
          ...
          <el-sub-menu :index="menuItem.code" v-else>
            ...
            <el-menu-item
              :index="item.path"
              v-for="item in menuItem.child_menu"
            >
              <el-icon style="margin-left: 10px">
    ...
    >
      <el-form :model="pwdFormData" ref="pwdFormRef">
        ...
        <el-form-item prop="confirm_password" required>
          <el-input
            v-model="pwdFormData.confirm_password"
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="changePwdDialogVisible = false" :icon="Ban"
            >取 消</el-button
          >
          <el-button
            type="primary"
            @click="handleSubmitChangePwd(pwdFormRef)"
            :icon="Send"
            >提 交</el-button
          >
        </div>
...
const handleLogout = () => {
  ...
  userStatuStore.role = "Common_user";
};
// 控制修改密码Dialog是否显示
...
const handleSubmitChangePwd = async (pwdFormRefEl: any) => {
  ...
  pwdFormRefEl.validate((result: boolean) => {
    ...
    const changeFunc =
      role === "Common_user"
        ? UserLogin.common_userChangePasswordApi
        : UserLogin.adminChangePasswordApi;
    // 调用修改密码函数，并根据返回结果进行相应处理
    changeFunc(JSON.parse(userInfo)?.userId, pwdFormData.value).then(
      (response) => {
        if (response.code !== 200) {
